...
export async function start(method = "periodic", elements, canvas, samples = 10) {
	...
	if (!(method in methods)) throw 'Method not available for WebGL!'
		...
		for (var counter = 0; counter < elements.length; counter++) {
			var darkColor = (elements[counter].getAttribute("data-dark-color") ?? '0,0,0,1').split(',').map(Number);
			var lightColor = (elements[counter].getAttribute("data-light-color") ?? '1,1,1,1').split(',').map(Number);
			var offScreenCanvases = webgl.setUpOffScreenCanvases(darkColor, lightColor);
